...
public:
  ...
  [[nodiscard]] std::int64_t get_vcpu(std::vector<virDomainVcpuFlags> flags={}) const;
  [[nodiscard]] std::double_t get_max_memory(datatype::SizeType size_unit=datatype::SizeType::KIB) const;
   ...
   * Setters
   * */
  std::int8_t set_vcpu(std::uint32_t nvcpus, std::vector<virDomainVcpuFlags> flags={});
  std::int8_t set_memory(std::uint64_t memory, datatype::SizeType size_unit=datatype::SizeType::BYTE, std::vector<virDomainMemoryModFlags> flags={});
}; /* LibvirtConnection */
} /* Impl */
} /* VirtXX */
