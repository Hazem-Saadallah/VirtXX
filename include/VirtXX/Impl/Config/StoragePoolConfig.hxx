...
namespace Config {
  class StoragePoolConfig : public Config::BaseConfig {
  public:
    enum class Field : std::uint8_t { NAME, TYPE, TARGET_PATH, };

  private:
    std::unordered_map<Config::StoragePoolConfig::Field, std::variant<ConfigValue<std::string>>> m_Values = {
      {Config::StoragePoolConfig::Field::TYPE, ConfigValue<std::string>({"pool", "@type"}, Config::Impl::Datatype::get_storage_pool_type_string(Config::Impl::Datatype::StoragePoolType::DIRECTORY))},
      {Config::StoragePoolConfig::Field::NAME, ConfigValue<std::string>({"pool", "name"}, Misc::generate_random_string(10, "VirtXX_POOL_", ""))},
      {Config::StoragePoolConfig::Field::TARGET_PATH ,ConfigValue<std::string>({"pool", "target", "path"}, std::filesystem::current_path().string())},
    };
